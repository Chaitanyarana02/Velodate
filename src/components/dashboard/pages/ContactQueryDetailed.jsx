...
const ContactQueryDetailed = () => {
  ...
  const { singleQueryData } = location.state;

  console.log(singleQueryData);
        ...
        <div className="w-full flex flex-col md:flex-row md:justify-between md:items-center">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-[#F6F6F6] text-md">
                  {singleQueryData.city || "Los Angles"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-8 w-full mt-8 md:mt-0">
            <div className="col-span-1 max-[549px]:col-span-2 max-[340px]:col-span-3 text-[#FFFFFF] text-[14px] w-2/6 max-[549px]:w-1/2">
              {singleQueryData.status.toLowerCase() === "ACTIVE" ? (
                <span className="flex justify-between items-center cursor-pointer p-2 bg-[#CD256133] rounded-full text-center border-2 border-[#CD2561]">
                  {singleQueryData.status}
                  <IoIosArrowDown className="mr-2" />
                </span>
              ) : (
                <span className="flex justify-between items-center cursor-pointer p-2 bg-[#25CD7C33] rounded-full text-center border-2 border-[#25CD7C33]">
                  {singleQueryData.status}
                  <IoIosArrowDown className="mr-2" />
                </span>
              )}
            ...
            <div className="flex gap-12 items-center my-4">
              ...
              <div className="flex flex-col gap-6">
                <h1 className="m-0 p-0 text-[16px]">
                  {singleQueryData.gender}
                </h1>
                <h1 className="m-0 p-0 text-[16px]">
                  {singleQueryData.moblie || "+91 9975879857"}
                </h1>
                <h1 className="m-0 p-0 text-[16px]">
                  {singleQueryData.email || "example@gmail.com"}
                </h1>

                <h1 className="m-0 p-0 text-[16px]">
                  {Array.isArray(singleQueryData.whoWouldYouLikeToDate) &&
                  singleQueryData.whoWouldYouLikeToDate.length > 0
                    ? singleQueryData.whoWouldYouLikeToDate.join(", ")
                    : singleQueryData.whoWouldYouLikeToDate || "Girl"}
                </h1>

                <h1 className="m-0 p-0 text-[16px]">
                  {Array.isArray(singleQueryData.thingsUsersLove) &&
                  singleQueryData.thingsUsersLove.length > 0
                    ? singleQueryData.thingsUsersLove.join(", ")
                    : singleQueryData.thingsUsersLove || "Girl"}
                </h1>
              </div>
            </div>
          </div>
